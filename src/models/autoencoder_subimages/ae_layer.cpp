...
namespace ML::models::autoencoder_fixed {
	...
	struct subimage {
			...
			signal(odim)
		{
			...
			int n_weights = 0;
			if(pattern.type == LAYER_TYPE::ENCODE) n_weights = pattern.A * pattern.A * idim.innerC();
			if(pattern.type == LAYER_TYPE::SPATIAL_MIX) n_weights = pattern.N * pattern.N;
			assert(n_weights > 0);
			fw_targets.resize(n_weights);
			bp_targets.resize(n_weights);
		}
