...
namespace ML::models::autoencoder_subimage {
	struct ae_layer {
		...
		static void back_propagate_func(ae_layer& layer, const int beg, const int end, simd_image_8f& error_i, const simd_image_8f& value_i, const float adjustment_rate_w, const float adjustment_rate_b) {
			for(int x=beg;x<end;x++) layer.subimages[x].backward_propagate(error_i, layer.error_o, value_i, layer.signal_o, adjustment_rate_w, adjustment_rate_b);
		}
		void back_propagate(const int n_threads, simd_image_8f& error_i, const simd_image_8f& value_i, const float adjustment_rate_w, const float adjustment_rate_b) {
			vector<std::thread> threads;
			for(int z=0;z<n_threads;z++) {
				const int beg = (subimages.size() * (z+0)) / n_threads;
				const int end = (subimages.size() * (z+1)) / n_threads;
				threads.push_back(std::thread(back_propagate_func, std::ref(*this), beg, end, std::ref(error_i), std::ref(value_i), adjustment_rate_w, adjustment_rate_b));
			}
			for(int z=0;z<n_threads;z++) threads[z].join();
