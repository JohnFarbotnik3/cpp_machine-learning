...
namespace ML::models::autoencoder_fixed {
	...
	struct bp_target { float weight=0.0f, weight_error=0.0f; };

	enum LAYER_TYPE {
		// scale image from AxA squares to BxB squares, mixing colour channels as well.
		ENCODE,
		// mix pixels (per-channel) from centered-NxN squares to centered-MxM squares, leaving image size the same.
		SPATIAL_MIX,
	};
	...
	struct ae_layer {
		vector<image_t> biases;
		vector<image_t> biases_error;// accumulated error in biases during minibatch.
		vector<image_t> input;
		vector<image_t> output;// image of output values - used for backprop.
		vector<image_t> signal;// image of signal values - used for backprop.
		vector<vector<fw_target>> fw_targets;// foreward-propagation targets.
		vector<vector<bp_target>> bp_targets;// backward-propagation targets.
