...
namespace ML::image::value_image_padded {
	...
	struct value_image_padded_iterator {
		...
		value_image_padded_iterator() = default;
		value_image_padded_iterator(
			value_image_padded_dimensions dim,
			int x0, int x1,
			int y0, int y1,
			int c0, int c1
	...
	struct value_image_padded {
		...
		value_image_padded_iterator get_iterator(
			int x0, int x1,
			int y0, int y1,
			int c0, int c1
		) const {
			return value_image_padded_iterator(dim, x0, x1, y0, y1, c0, c1);
		}
	};

	void sample_area_copy(value_image_padded<float>& sample, const file_image& image) {
		...
		value_image_padded_iterator sample_iter = sample.get_iterator(
			sample.sx0, sample.sx1,
			sample.sy0, sample.sy1,
			0, image.C
		);
		...
		while(sample_iter.has_next()) {
			int ofs = image.get_offset(
				sample_iter.x - sample.sx0,
				sample_iter.y - sample.sy0,
				sample_iter.c
			);
	...
	void sample_area_nearest_neighbour(value_image_padded<float>& sample, const file_image& image) {
		sample.clear();
		...
		const float m = 1.0f / 255.0f;
		const float x_inv_scale_factor = float(image.X) / float(sample.sx1 - sample.sx0);
		const float y_inv_scale_factor = float(image.Y) / float(sample.sy1 - sample.sy0);
		while(sample_iter.has_next()) {
			int ix = std::clamp(int(float(sample_iter.x - sample.sx0) * x_inv_scale_factor), 0, image.X);
			int iy = std::clamp(int(float(sample_iter.y - sample.sy0) * y_inv_scale_factor), 0, image.Y);
			int ofs = image.get_offset(ix, iy, sample_iter.c);
			sample.data[sample_iter.i] = image.data[ofs] * m;
			sample_iter.next();
		}
	}

	void sample_area_linear(value_image_padded<float>& sample, const file_image& image) {
		sample.clear();

		// compute float conversions between sample coordinates to image coordinates.
		vector<int> floor_mx(sample.dim.innerX()+2);
		vector<int> floor_my(sample.dim.innerY()+2);
		vector<int> ceil__mx(sample.dim.innerX()+2);
		vector<int> ceil__my(sample.dim.innerY()+2);
		...
		{
			float mx = float(image.X) / float(sample.sx1 - sample.sx0);
			float my = float(image.Y) / float(sample.sy1 - sample.sy0);
	...
	*/
	void generate_sample_image(value_image_padded<float>& sample, const file_image& image) {
		const int sample_X = sample.dim.innerX();
		const int sample_Y = sample.dim.innerY();
		...
		if(image.X <= sample_X && image.Y <= sample_Y) {
			...
			sample.sx0 = remaining_w/2;
			sample.sy0 = remaining_h/2;
			sample.sx1 = sample.sx0 + image.X;
			sample.sy1 = sample.sy0 + image.Y;
		...
		// if loaded image is larger than sample area, then scale down.
			...
			if(iw_over_sw >= ih_over_sh) {
				...
				sample.sy0 = remaining_h / 2;
				sample.sy1 = (sample.sy0 + scaled_h);
			} else {
				...
				sample.sx0 = remaining_w / 2;
				sample.sx1 = (sample.sx0 + scaled_w);
			}
	...
	file_image to_file_image(value_image_padded<float>& sample, bool clamp_to_sample_area) {
		file_image image;
